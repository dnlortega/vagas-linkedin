...
export function useVagasFiltros() {
  ...
  function salvarFiltroAtual() {
    ...
    if (!nome?.trim()) return;
    const preset = { nome: nome.trim(), filtro, senioridade, modalidade, techFiltro, fonteFiltro, modoTrabalho, periodo, ordem, busca };
  ...
  function restaurarFiltro(preset) {
    ...
    setFonteFiltro(preset.fonteFiltro || 'todas');
