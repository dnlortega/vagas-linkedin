...
export function tipoLocalidade(local, cidadeUser = 'bauru') {
  const l = (local || '').toLowerCase();
  const c = (cidadeUser || 'bauru').toLowerCase();
  if (c && l.includes(c)) return 'bauru'; // 'bauru' acts as the internal ID for "Minha Cidade"
  if (l.includes('bauru')) return 'bauru'; // fallback if they haven't set it and it's actually Bauru
  if (CIDADES.some(cd => l.includes(cd))) return 'regiao';
